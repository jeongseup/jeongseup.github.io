---
title: "EVM은 왜 정수(uint)만 지원하는가?"
date: 2026-02-25
draft: false
authors: ["Jeongseup"]
description: "Solidity에 float이 없는 이유와 블록체인 합의의 관계. 부동소수점이 합의를 깨뜨리는 구조적 원인부터 DeFi의 우회 방법까지."
slug: evm-integer-only-design
tags: ["EVM", "Solidity", "Blockchain", "DeFi", "Smart Contract"]
categories: []
math: true
---
...
블록체인은 수천 개 노드가 동일한 계산을 수행하고 **같은 결과에 합의**해야 하는 시스템이다.

그런데 IEEE 754 부동소수점(`float`/`double`)은 하드웨어마다 미세하게 다른 결과를 낼 수 있다:
...
컴파일러 최적화가 연산 순서를 바꾸면 결과가 달라진다.

### 특수값 처리 차이

`NaN`, `Infinity`, `-0`, denormalized numbers 등 특수값들의 비교·연산 동작이 구현마다 미세하게 다르다.
...
EVM은 **256비트 워드 기반 스택 머신**이다. `uint256` 하나로 통일하면:
...
복잡도 대비 이점이 없다. 정수 스케일링으로 충분하기 때문이다.

---

## 다른 블록체인도 마찬가지인가?

L2도 `float`을 명시적으로 제거한다:
...
| Move (Sui/Aptos) | `u8`/`u64`/`u128`/`u256`만 지원, float 없음 |

**블록체인 VM에서 float 배제는 업계 표준**이다.
...
| 방식 | 사용 사례 |
|------|----------|
| **1e18 (Mantissa)** | Compound — 범용 비율 계산 |
...
EVM이 `uint`만 지원하는 이유를 우선순위로 정리하면:
...
분산 합의가 필요 없는 시스템(게임 엔진, 과학 계산)에서는 float이 적합하지만, **"수천 대 컴퓨터가 같은 답을 내야 하는" 블록체인에서는 정수가 유일한 선택**이다.
